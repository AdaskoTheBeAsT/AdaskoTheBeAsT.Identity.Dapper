...
$DatabaseName = "WithoutNormalizedAspNetIdentityGuid"
...
dotnet build "./WithoutNormalizedAspNetIdentityGuid.sqlproj" -p:Configuration=Debug

# prepare publish script
dotnet sqlpackage /Action:Script /SourceFile:"./bin/Debug/WithoutNormalizedAspNetIdentityGuid.dacpac" `
    /OutputPath:"./bin/Debug/WithoutNormalizedAspNetIdentityGuid.publish.sql" `
    /TargetConnectionString:"$ConnectionStringTest"

# deploy to local sql server project to check if everything is ok
dotnet sqlpackage /Action:Publish /SourceFile:"./bin/Debug/WithoutNormalizedAspNetIdentityGuid.dacpac" `
    /TargetConnectionString:"$ConnectionStringTest"

# copy init script to integ test
Copy-Item "./bin/Debug/WithoutNormalizedAspNetIdentityGuid.publish.sql" "../../../test/integ/AdaskoTheBeAsT.Identity.Dapper.SqlServer.IntegrationTest/Scripts/WithoutNormalizedAspNetIdentityGuid/init.sql" -Force
