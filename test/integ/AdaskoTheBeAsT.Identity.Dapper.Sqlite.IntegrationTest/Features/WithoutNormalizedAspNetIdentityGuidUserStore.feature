...
Scenario Outline: Verify lightweight UserStore method <method> without normalized and Guid id
    ...
    When I verify "<method>" on SQLite UserStore without normalized and Guid id
    Then "<method>" on SQLite UserStore should work without normalized and Guid id
...
Scenario: Query Users with Users without normalized and Guid id
    Given I have configured SQLite UserStore without normalized and Guid id
    And I created users for SQLite UserStore
        | UserName | Email             |
        | John     | john@example.com  |
    And I created roles for SQLite UserStore
        | Name  |
        | Admin |
    And I added users to roles for SQLite UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "Users" on SQLite UserStore
...
Scenario: Create user with CreateAsync without normalized and Guid id
    Given I have configured SQLite UserStore without normalized and Guid id
    When I execute "CreateAsync" on SQLite UserStore with parameters
        | UserName | Email            |
        | John     | john@example.com |
    Then the last identity result for SQLite UserStore should be successful
    And the last user result for SQLite UserStore should match
        | UserName | Email            |
        | John     | john@example.com |
...
Scenario: Update user with UpdateAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "UpdateAsync" on SQLite UserStore with parameters
...
Scenario: Delete user with DeleteAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "DeleteAsync" on SQLite UserStore with parameters
        | UserName |
        | John     |
    Then the last identity result for SQLite UserStore should be successful
    And the last user result for SQLite UserStore should be null
...
Scenario: Find user with FindByIdAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "FindByIdAsync" on SQLite UserStore with parameters
        | UserName |
        | John     |
...
Scenario: Find user with FindByNameAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "FindByNameAsync" on SQLite UserStore with parameters
        | UserName |
        | John     |
    Then the last user result for SQLite UserStore should match
        | UserName | Email            |
...
Scenario: Find user with FindByEmailAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "FindByEmailAsync" on SQLite UserStore with parameters
        | Email            |
        | john@example.com |
    Then the last user result for SQLite UserStore should match
        | UserName | Email            |
...
Scenario: Get user claims with GetClaimsAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    And I added user claims for SQLite UserStore
        | UserName | ClaimType  | ClaimValue |
        | John     | permission | read       |
        | John     | permission | write      |
    When I execute "GetClaimsAsync" on SQLite UserStore with parameters
        | UserName |
        | John     |
...
Scenario: Add user claims with AddClaimsAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "AddClaimsAsync" on SQLite UserStore with parameters
        | UserName | ClaimType  | ClaimValue |
        | John     | permission | read       |
        | John     | permission | write      |
    Then the last claims result for SQLite UserStore should match
...
Scenario: Replace user claim with ReplaceClaimAsync without normalized and Guid id
    ...
    And I added user claims for SQLite UserStore
        | UserName | ClaimType  | ClaimValue |
        | John     | permission | read       |
    When I execute "ReplaceClaimAsync" on SQLite UserStore with parameters
...
Scenario: Remove user claims with RemoveClaimsAsync without normalized and Guid id
    ...
    And I added user claims for SQLite UserStore
        | UserName | ClaimType  | ClaimValue |
        | John     | permission | read       |
    When I execute "RemoveClaimsAsync" on SQLite UserStore with parameters
        | UserName | ClaimType  | ClaimValue |
        | John     | permission | read       |
    Then the last claims result for SQLite UserStore should match
        | ClaimType | ClaimValue |
...
Scenario: Add user login with AddLoginAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "AddLoginAsync" on SQLite UserStore with parameters
...
Scenario: Remove user login with RemoveLoginAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    And I added user logins for SQLite UserStore
        | UserName | LoginProvider | ProviderKey | ProviderDisplayName |
        | John     | GitHub        | john-gh     | GitHub              |
    When I execute "RemoveLoginAsync" on SQLite UserStore with parameters
...
Scenario: Get user logins with GetLoginsAsync without normalized and Guid id
    ...
    And I added user logins for SQLite UserStore
        | UserName | LoginProvider | ProviderKey | ProviderDisplayName |
        | John     | GitHub        | john-gh     | GitHub              |
    When I execute "GetLoginsAsync" on SQLite UserStore with parameters
        | UserName |
        | John     |
    Then the last logins result for SQLite UserStore should match
        | LoginProvider | ProviderKey | ProviderDisplayName |
...
Scenario: Find user by login with FindByLoginAsync without normalized and Guid id
    ...
    And I added user logins for SQLite UserStore
        | UserName | LoginProvider | ProviderKey | ProviderDisplayName |
        | John     | GitHub        | john-gh     | GitHub              |
    When I execute "FindByLoginAsync" on SQLite UserStore with parameters
...
Scenario: Increment access failed count with IncrementAccessFailedCountAsync without normalized and Guid id
    ...
    When I execute "IncrementAccessFailedCountAsync" on SQLite UserStore with parameters
        | UserName |
        | John     |
    Then the last integer result for SQLite UserStore should be 1
    And the last user result for SQLite UserStore should match
...
Scenario: Get users for claim with GetUsersForClaimAsync without normalized and Guid id
    ...
    And I added user claims for SQLite UserStore
        | UserName | ClaimType  | ClaimValue |
        | John     | permission | read       |
    When I execute "GetUsersForClaimAsync" on SQLite UserStore with parameters
        | ClaimType  | ClaimValue |
        | permission | read       |
    Then the last users result for SQLite UserStore should match
        | UserName | Email            |
...
Scenario: Set token with SetTokenAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "SetTokenAsync" on SQLite UserStore with parameters
        | UserName | LoginProvider | Name          | Value         |
        | John     | api           | refresh-token | refresh-value |
    Then the last string result for SQLite UserStore should be "refresh-value"
...
Scenario: Remove token with RemoveTokenAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    And I added user tokens for SQLite UserStore
        | UserName | LoginProvider | Name          | Value         |
        | John     | api           | refresh-token | refresh-value |
    When I execute "RemoveTokenAsync" on SQLite UserStore with parameters
...
Scenario: Get token with GetTokenAsync without normalized and Guid id
    ...
    And I added user tokens for SQLite UserStore
        | UserName | LoginProvider | Name          | Value         |
        | John     | api           | refresh-token | refresh-value |
    When I execute "GetTokenAsync" on SQLite UserStore with parameters
        | UserName | LoginProvider | Name          |
        | John     | api           | refresh-token |
    Then the last string result for SQLite UserStore should be "refresh-value"
...
Scenario: Set authenticator key with SetAuthenticatorKeyAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "SetAuthenticatorKeyAsync" on SQLite UserStore with parameters
        | UserName | Key      |
        | John     | auth-key |
    Then the last string result for SQLite UserStore should be "auth-key"
...
Scenario: Get authenticator key with GetAuthenticatorKeyAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    And I set authenticator keys for SQLite UserStore
        | UserName | Key      |
        | John     | auth-key |
    When I execute "GetAuthenticatorKeyAsync" on SQLite UserStore with parameters
        | UserName |
        | John     |
    Then the last string result for SQLite UserStore should be "auth-key"
...
Scenario: Count recovery codes with CountCodesAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    And I replaced recovery codes for SQLite UserStore
        | UserName | RecoveryCodes          |
        | John     | code-1;code-2;code-3  |
    When I execute "CountCodesAsync" on SQLite UserStore with parameters
        | UserName |
        | John     |
    Then the last integer result for SQLite UserStore should be 3
...
Scenario: Replace recovery codes with ReplaceCodesAsync without normalized and Guid id
    ...
    And I created users for SQLite UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "ReplaceCodesAsync" on SQLite UserStore with parameters
        | UserName | RecoveryCodes          |
        | John     | code-1;code-2;code-3  |
    Then the last integer result for SQLite UserStore should be 3
...
Scenario: Redeem recovery code with RedeemCodeAsync without normalized and Guid id
    ...
    And I replaced recovery codes for SQLite UserStore
        | UserName | RecoveryCodes          |
        | John     | code-1;code-2;code-3  |
    When I execute "RedeemCodeAsync" on SQLite UserStore with parameters
        | UserName | Code   |
        | John     | code-1 |
    Then the last boolean result for SQLite UserStore should be "True"
...
Scenario: Get users in role with GetUsersInRoleAsync without normalized and Guid id
    ...
    And I added users to roles for SQLite UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "GetUsersInRoleAsync" on SQLite UserStore with parameters
        | RoleName |
        | Admin    |
    Then the last users result for SQLite UserStore should match
        | UserName | Email            |
...
Scenario: Add user to role with AddToRoleAsync without normalized and Guid id
    ...
    And I created roles for SQLite UserStore
        | Name  |
        | Admin |
    When I execute "AddToRoleAsync" on SQLite UserStore with parameters
        | UserName | RoleName |
        | John     | Admin    |
...
Scenario: Remove user from role with RemoveFromRoleAsync without normalized and Guid id
    ...
    And I added users to roles for SQLite UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "RemoveFromRoleAsync" on SQLite UserStore with parameters
        | UserName | RoleName |
        | John     | Admin    |
    Then the last strings result for SQLite UserStore should match
        | Name |
...
Scenario: Get roles for user with GetRolesAsync without normalized and Guid id
    ...
    And I added users to roles for SQLite UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "GetRolesAsync" on SQLite UserStore with parameters
        | UserName |
        | John     |
    Then the last strings result for SQLite UserStore should match
        | Name  |
...
Scenario: Check role membership with IsInRoleAsync without normalized and Guid id
    ...
    And I added users to roles for SQLite UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "IsInRoleAsync" on SQLite UserStore with parameters
        | UserName | RoleName |
        | John     | Admin    |
    Then the last boolean result for SQLite UserStore should be "True"
...
Scenario: Get role claims for user with GetRoleClaimsAsync without normalized and Guid id
    ...
    And I created roles for SQLite UserStore
        | Name  |
        | Admin |
    And I added role claims for SQLite UserStore
        | RoleName | ClaimType  | ClaimValue   |
        | Admin    | permission | manage-users |
    And I added users to roles for SQLite UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "GetRoleClaimsAsync" on SQLite UserStore with parameters
        | UserName |
        | John     |
    Then the last claims result for SQLite UserStore should match
        | ClaimType  | ClaimValue   |
...
Scenario: Get user and role claims with GetUserAndRoleClaimsAsync without normalized and Guid id
    ...
    And I added users to roles for SQLite UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "GetUserAndRoleClaimsAsync" on SQLite UserStore with parameters
        | UserName |
        | John     |
    Then the last claims result for SQLite UserStore should match
        | ClaimType  | ClaimValue   |
