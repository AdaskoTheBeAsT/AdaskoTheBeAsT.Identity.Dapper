...
public abstract class SqlServerUserStoreTableDrivenStepDefinitionsBase
    : SqlServerStoreIntegrationTestBase
{
    ...
    protected ApplicationUser? LastUser { get; set; }

    protected IList<ApplicationUser> LastUsers { get; set; } = new List<ApplicationUser>();
    ...
    protected abstract SqlServerUserOnlyStoreBase CreateUserStoreInstance();

    protected virtual SqlServerUserStoreBase CreateRoleCapableUserStoreInstance() =>
    ...
    protected Task ResetUserStoreScenarioAsync()
    {
        ...
        LastUser = null;
        LastUsers = new List<ApplicationUser>();
    ...
    protected void AssertLastClaimsMatch(Table table)
    {
        var expected = table.Rows.Select(
            row => (GetRequiredValue(table, row, "ClaimType"), MakeRequiredScenarioUniqueValue(GetRequiredValue(table, row, "ClaimValue"))));
    ...
    private async Task ExecuteFindUserByNameAsync(Table table)
    {
        using var store = CreateUserStoreInstance();
        var row = RequireSingleRow(table);
        LastUser = await store.FindByNameAsync(MakeRequiredScenarioUniqueValue(GetRequiredValue(table, row, "UserName")), CancellationToken.None);
    }
    ...
    private async Task ExecuteFindUserByEmailAsync(Table table)
    {
        using var store = CreateUserStoreInstance();
        var row = RequireSingleRow(table);
        LastUser = await store.FindByEmailAsync(MakeScenarioUniqueEmail(GetRequiredValue(table, row, "Email")), CancellationToken.None);
    }
    ...
    private async Task ExecuteReplaceClaimAsync(Table table)
    {
        ...
        await store.ReplaceClaimAsync(
            user,
            new Claim(GetRequiredValue(table, row, "ClaimTypeOld"), MakeRequiredScenarioUniqueValue(GetRequiredValue(table, row, "ClaimValueOld"))),
            new Claim(GetRequiredValue(table, row, "ClaimTypeNew"), MakeRequiredScenarioUniqueValue(GetRequiredValue(table, row, "ClaimValueNew"))),
            CancellationToken.None);
        LastClaims = await store.GetClaimsAsync(user, CancellationToken.None);
    }
    ...
    private async Task ExecuteSetAuthenticatorKeyAsync(Table table)
    {
        ...
        var row = RequireSingleRow(table);
        var user = GetUser(table, row);
        await store.SetAuthenticatorKeyAsync(user, MakeRequiredScenarioUniqueValue(GetRequiredValue(table, row, "Key")), CancellationToken.None);
    ...
    private async Task ExecuteGetUsersInRoleAsync(Table table)
    {
        using var store = CreateRoleCapableUserStoreInstance();
        var row = RequireSingleRow(table);
        LastUsers = await store.GetUsersInRoleAsync(MakeRequiredScenarioUniqueValue(GetRequiredValue(table, row, "RoleName")), CancellationToken.None);
    }
    ...
    private ApplicationRole GetRole(Table table, DataTableRow row)
    {
        var roleName = MakeRequiredScenarioUniqueValue(GetRequiredValue(table, row, "RoleName"));
    ...
    private void ApplyUserUpdates(Table table, DataTableRow row, ApplicationUser user)
    {
    ...
    private Claim CreateClaim(Table table, DataTableRow row) =>
        new(GetRequiredValue(table, row, "ClaimType"), MakeRequiredScenarioUniqueValue(GetRequiredValue(table, row, "ClaimValue")));
    ...
    private ApplicationUser ResolveUserFromCollection(IEnumerable<ApplicationUser> users, Table table, DataTableRow row)
    {
        ...
        if (GetOptionalValue(table, row, "UserName") is { } userName)
        {
            ...
            return users.Single(user => string.Equals(user.UserName, scenarioUserName, StringComparison.OrdinalIgnoreCase));
        }

        return users.Single();
    }

    private void AssertUserMatches(ApplicationUser actual, Table table, DataTableRow row)
    {
