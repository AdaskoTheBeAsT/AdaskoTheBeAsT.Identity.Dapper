...
Scenario Outline: Verify lightweight UserOnlyStore method <method> without normalized and Guid id
	...
	When I verify "<method>" on SQL Server UserOnlyStore without normalized and Guid id
	Then "<method>" on SQL Server UserOnlyStore should work without normalized and Guid id
...
Scenario: Query Users with Users without normalized and Guid id
	Given I have configured SQL Server UserOnlyStore without normalized and Guid id
	And I created users for SQL Server UserOnlyStore
		| UserName | Email             |
		| John     | john@example.com  |
	And I created roles for SQL Server UserOnlyStore
		| Name  |
		| Admin |
	And I added users to roles for SQL Server UserOnlyStore
		| UserName | RoleName |
		| John     | Admin    |
	When I execute "Users" on SQL Server UserOnlyStore
...
Scenario: Create user with CreateAsync without normalized and Guid id
	Given I have configured SQL Server UserOnlyStore without normalized and Guid id
	When I execute "CreateAsync" on SQL Server UserOnlyStore with parameters
		| UserName | Email            |
		| John     | john@example.com |
	Then the last identity result for SQL Server UserOnlyStore should be successful
	And the last user result for SQL Server UserOnlyStore should match
		| UserName | Email            |
		| John     | john@example.com |
...
Scenario: Update user with UpdateAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	When I execute "UpdateAsync" on SQL Server UserOnlyStore with parameters
...
Scenario: Delete user with DeleteAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	When I execute "DeleteAsync" on SQL Server UserOnlyStore with parameters
		| UserName |
		| John     |
	Then the last identity result for SQL Server UserOnlyStore should be successful
	And the last user result for SQL Server UserOnlyStore should be null
...
Scenario: Find user with FindByIdAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	When I execute "FindByIdAsync" on SQL Server UserOnlyStore with parameters
		| UserName |
		| John     |
...
Scenario: Find user with FindByNameAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	When I execute "FindByNameAsync" on SQL Server UserOnlyStore with parameters
		| UserName |
		| John     |
	Then the last user result for SQL Server UserOnlyStore should match
		| UserName | Email            |
...
Scenario: Find user with FindByEmailAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	When I execute "FindByEmailAsync" on SQL Server UserOnlyStore with parameters
		| Email            |
		| john@example.com |
	Then the last user result for SQL Server UserOnlyStore should match
		| UserName | Email            |
...
Scenario: Get user claims with GetClaimsAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	And I added user claims for SQL Server UserOnlyStore
		| UserName | ClaimType  | ClaimValue |
		| John     | permission | read       |
		| John     | permission | write      |
	When I execute "GetClaimsAsync" on SQL Server UserOnlyStore with parameters
		| UserName |
		| John     |
...
Scenario: Add user claims with AddClaimsAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	When I execute "AddClaimsAsync" on SQL Server UserOnlyStore with parameters
		| UserName | ClaimType  | ClaimValue |
		| John     | permission | read       |
		| John     | permission | write      |
	Then the last claims result for SQL Server UserOnlyStore should match
...
Scenario: Replace user claim with ReplaceClaimAsync without normalized and Guid id
	...
	And I added user claims for SQL Server UserOnlyStore
		| UserName | ClaimType  | ClaimValue |
		| John     | permission | read       |
	When I execute "ReplaceClaimAsync" on SQL Server UserOnlyStore with parameters
...
Scenario: Remove user claims with RemoveClaimsAsync without normalized and Guid id
	...
	And I added user claims for SQL Server UserOnlyStore
		| UserName | ClaimType  | ClaimValue |
		| John     | permission | read       |
	When I execute "RemoveClaimsAsync" on SQL Server UserOnlyStore with parameters
		| UserName | ClaimType  | ClaimValue |
		| John     | permission | read       |
	Then the last claims result for SQL Server UserOnlyStore should match
		| ClaimType | ClaimValue |
...
Scenario: Add user login with AddLoginAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	When I execute "AddLoginAsync" on SQL Server UserOnlyStore with parameters
...
Scenario: Remove user login with RemoveLoginAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	And I added user logins for SQL Server UserOnlyStore
		| UserName | LoginProvider | ProviderKey | ProviderDisplayName |
		| John     | GitHub        | john-gh     | GitHub              |
	When I execute "RemoveLoginAsync" on SQL Server UserOnlyStore with parameters
...
Scenario: Get user logins with GetLoginsAsync without normalized and Guid id
	...
	And I added user logins for SQL Server UserOnlyStore
		| UserName | LoginProvider | ProviderKey | ProviderDisplayName |
		| John     | GitHub        | john-gh     | GitHub              |
	When I execute "GetLoginsAsync" on SQL Server UserOnlyStore with parameters
		| UserName |
		| John     |
	Then the last logins result for SQL Server UserOnlyStore should match
		| LoginProvider | ProviderKey | ProviderDisplayName |
...
Scenario: Find user by login with FindByLoginAsync without normalized and Guid id
	...
	And I added user logins for SQL Server UserOnlyStore
		| UserName | LoginProvider | ProviderKey | ProviderDisplayName |
		| John     | GitHub        | john-gh     | GitHub              |
	When I execute "FindByLoginAsync" on SQL Server UserOnlyStore with parameters
...
Scenario: Increment access failed count with IncrementAccessFailedCountAsync without normalized and Guid id
	...
	When I execute "IncrementAccessFailedCountAsync" on SQL Server UserOnlyStore with parameters
		| UserName |
		| John     |
	Then the last integer result for SQL Server UserOnlyStore should be 1
	And the last user result for SQL Server UserOnlyStore should match
...
Scenario: Get users for claim with GetUsersForClaimAsync without normalized and Guid id
	...
	And I added user claims for SQL Server UserOnlyStore
		| UserName | ClaimType  | ClaimValue |
		| John     | permission | read       |
	When I execute "GetUsersForClaimAsync" on SQL Server UserOnlyStore with parameters
		| ClaimType  | ClaimValue |
		| permission | read       |
	Then the last users result for SQL Server UserOnlyStore should match
		| UserName | Email            |
...
Scenario: Set token with SetTokenAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	When I execute "SetTokenAsync" on SQL Server UserOnlyStore with parameters
		| UserName | LoginProvider | Name          | Value         |
		| John     | api           | refresh-token | refresh-value |
	Then the last string result for SQL Server UserOnlyStore should be "refresh-value"
...
Scenario: Remove token with RemoveTokenAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	And I added user tokens for SQL Server UserOnlyStore
		| UserName | LoginProvider | Name          | Value         |
		| John     | api           | refresh-token | refresh-value |
	When I execute "RemoveTokenAsync" on SQL Server UserOnlyStore with parameters
...
Scenario: Get token with GetTokenAsync without normalized and Guid id
	...
	And I added user tokens for SQL Server UserOnlyStore
		| UserName | LoginProvider | Name          | Value         |
		| John     | api           | refresh-token | refresh-value |
	When I execute "GetTokenAsync" on SQL Server UserOnlyStore with parameters
		| UserName | LoginProvider | Name          |
		| John     | api           | refresh-token |
	Then the last string result for SQL Server UserOnlyStore should be "refresh-value"
...
Scenario: Set authenticator key with SetAuthenticatorKeyAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	When I execute "SetAuthenticatorKeyAsync" on SQL Server UserOnlyStore with parameters
		| UserName | Key      |
		| John     | auth-key |
	Then the last string result for SQL Server UserOnlyStore should be "auth-key"
...
Scenario: Get authenticator key with GetAuthenticatorKeyAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	And I set authenticator keys for SQL Server UserOnlyStore
		| UserName | Key      |
		| John     | auth-key |
	When I execute "GetAuthenticatorKeyAsync" on SQL Server UserOnlyStore with parameters
		| UserName |
		| John     |
	Then the last string result for SQL Server UserOnlyStore should be "auth-key"
...
Scenario: Count recovery codes with CountCodesAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	And I replaced recovery codes for SQL Server UserOnlyStore
		| UserName | RecoveryCodes          |
		| John     | code-1;code-2;code-3  |
	When I execute "CountCodesAsync" on SQL Server UserOnlyStore with parameters
		| UserName |
		| John     |
	Then the last integer result for SQL Server UserOnlyStore should be 3
...
Scenario: Replace recovery codes with ReplaceCodesAsync without normalized and Guid id
	...
	And I created users for SQL Server UserOnlyStore
		| UserName | Email            |
		| John     | john@example.com |
	When I execute "ReplaceCodesAsync" on SQL Server UserOnlyStore with parameters
		| UserName | RecoveryCodes          |
		| John     | code-1;code-2;code-3  |
	Then the last integer result for SQL Server UserOnlyStore should be 3
...
Scenario: Redeem recovery code with RedeemCodeAsync without normalized and Guid id
	...
	And I replaced recovery codes for SQL Server UserOnlyStore
		| UserName | RecoveryCodes          |
		| John     | code-1;code-2;code-3  |
	When I execute "RedeemCodeAsync" on SQL Server UserOnlyStore with parameters
		| UserName | Code   |
		| John     | code-1 |
	Then the last boolean result for SQL Server UserOnlyStore should be "True"
