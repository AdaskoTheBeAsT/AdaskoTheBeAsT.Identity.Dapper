...
Scenario Outline: Verify lightweight UserStore method <method> without normalized and Guid id
    ...
    When I verify "<method>" on MySQL UserStore without normalized and Guid id
    Then "<method>" on MySQL UserStore should work without normalized and Guid id
...
Scenario: Query Users with Users without normalized and Guid id
    Given I have configured MySQL UserStore without normalized and Guid id
    And I created users for MySQL UserStore
        | UserName | Email             |
        | John     | john@example.com  |
    And I created roles for MySQL UserStore
        | Name  |
        | Admin |
    And I added users to roles for MySQL UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "Users" on MySQL UserStore
...
Scenario: Create user with CreateAsync without normalized and Guid id
    Given I have configured MySQL UserStore without normalized and Guid id
    When I execute "CreateAsync" on MySQL UserStore with parameters
        | UserName | Email            |
        | John     | john@example.com |
    Then the last identity result for MySQL UserStore should be successful
    And the last user result for MySQL UserStore should match
        | UserName | Email            |
        | John     | john@example.com |
...
Scenario: Update user with UpdateAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "UpdateAsync" on MySQL UserStore with parameters
...
Scenario: Delete user with DeleteAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "DeleteAsync" on MySQL UserStore with parameters
        | UserName |
        | John     |
    Then the last identity result for MySQL UserStore should be successful
    And the last user result for MySQL UserStore should be null
...
Scenario: Find user with FindByIdAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "FindByIdAsync" on MySQL UserStore with parameters
        | UserName |
        | John     |
...
Scenario: Find user with FindByNameAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "FindByNameAsync" on MySQL UserStore with parameters
        | UserName |
        | John     |
    Then the last user result for MySQL UserStore should match
        | UserName | Email            |
...
Scenario: Find user with FindByEmailAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "FindByEmailAsync" on MySQL UserStore with parameters
        | Email            |
        | john@example.com |
    Then the last user result for MySQL UserStore should match
        | UserName | Email            |
...
Scenario: Get user claims with GetClaimsAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    And I added user claims for MySQL UserStore
        | UserName | ClaimType  | ClaimValue |
        | John     | permission | read       |
        | John     | permission | write      |
    When I execute "GetClaimsAsync" on MySQL UserStore with parameters
        | UserName |
        | John     |
...
Scenario: Add user claims with AddClaimsAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "AddClaimsAsync" on MySQL UserStore with parameters
        | UserName | ClaimType  | ClaimValue |
        | John     | permission | read       |
        | John     | permission | write      |
    Then the last claims result for MySQL UserStore should match
...
Scenario: Replace user claim with ReplaceClaimAsync without normalized and Guid id
    ...
    And I added user claims for MySQL UserStore
        | UserName | ClaimType  | ClaimValue |
        | John     | permission | read       |
    When I execute "ReplaceClaimAsync" on MySQL UserStore with parameters
...
Scenario: Remove user claims with RemoveClaimsAsync without normalized and Guid id
    ...
    And I added user claims for MySQL UserStore
        | UserName | ClaimType  | ClaimValue |
        | John     | permission | read       |
    When I execute "RemoveClaimsAsync" on MySQL UserStore with parameters
        | UserName | ClaimType  | ClaimValue |
        | John     | permission | read       |
    Then the last claims result for MySQL UserStore should match
        | ClaimType | ClaimValue |
...
Scenario: Add user login with AddLoginAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "AddLoginAsync" on MySQL UserStore with parameters
...
Scenario: Remove user login with RemoveLoginAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    And I added user logins for MySQL UserStore
        | UserName | LoginProvider | ProviderKey | ProviderDisplayName |
        | John     | GitHub        | john-gh     | GitHub              |
    When I execute "RemoveLoginAsync" on MySQL UserStore with parameters
...
Scenario: Get user logins with GetLoginsAsync without normalized and Guid id
    ...
    And I added user logins for MySQL UserStore
        | UserName | LoginProvider | ProviderKey | ProviderDisplayName |
        | John     | GitHub        | john-gh     | GitHub              |
    When I execute "GetLoginsAsync" on MySQL UserStore with parameters
        | UserName |
        | John     |
    Then the last logins result for MySQL UserStore should match
        | LoginProvider | ProviderKey | ProviderDisplayName |
...
Scenario: Find user by login with FindByLoginAsync without normalized and Guid id
    ...
    And I added user logins for MySQL UserStore
        | UserName | LoginProvider | ProviderKey | ProviderDisplayName |
        | John     | GitHub        | john-gh     | GitHub              |
    When I execute "FindByLoginAsync" on MySQL UserStore with parameters
...
Scenario: Increment access failed count with IncrementAccessFailedCountAsync without normalized and Guid id
    ...
    When I execute "IncrementAccessFailedCountAsync" on MySQL UserStore with parameters
        | UserName |
        | John     |
    Then the last integer result for MySQL UserStore should be 1
    And the last user result for MySQL UserStore should match
...
Scenario: Get users for claim with GetUsersForClaimAsync without normalized and Guid id
    ...
    And I added user claims for MySQL UserStore
        | UserName | ClaimType  | ClaimValue |
        | John     | permission | read       |
    When I execute "GetUsersForClaimAsync" on MySQL UserStore with parameters
        | ClaimType  | ClaimValue |
        | permission | read       |
    Then the last users result for MySQL UserStore should match
        | UserName | Email            |
...
Scenario: Set token with SetTokenAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "SetTokenAsync" on MySQL UserStore with parameters
        | UserName | LoginProvider | Name          | Value         |
        | John     | api           | refresh-token | refresh-value |
    Then the last string result for MySQL UserStore should be "refresh-value"
...
Scenario: Remove token with RemoveTokenAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    And I added user tokens for MySQL UserStore
        | UserName | LoginProvider | Name          | Value         |
        | John     | api           | refresh-token | refresh-value |
    When I execute "RemoveTokenAsync" on MySQL UserStore with parameters
...
Scenario: Get token with GetTokenAsync without normalized and Guid id
    ...
    And I added user tokens for MySQL UserStore
        | UserName | LoginProvider | Name          | Value         |
        | John     | api           | refresh-token | refresh-value |
    When I execute "GetTokenAsync" on MySQL UserStore with parameters
        | UserName | LoginProvider | Name          |
        | John     | api           | refresh-token |
    Then the last string result for MySQL UserStore should be "refresh-value"
...
Scenario: Set authenticator key with SetAuthenticatorKeyAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "SetAuthenticatorKeyAsync" on MySQL UserStore with parameters
        | UserName | Key      |
        | John     | auth-key |
    Then the last string result for MySQL UserStore should be "auth-key"
...
Scenario: Get authenticator key with GetAuthenticatorKeyAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    And I set authenticator keys for MySQL UserStore
        | UserName | Key      |
        | John     | auth-key |
    When I execute "GetAuthenticatorKeyAsync" on MySQL UserStore with parameters
        | UserName |
        | John     |
    Then the last string result for MySQL UserStore should be "auth-key"
...
Scenario: Count recovery codes with CountCodesAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    And I replaced recovery codes for MySQL UserStore
        | UserName | RecoveryCodes          |
        | John     | code-1;code-2;code-3  |
    When I execute "CountCodesAsync" on MySQL UserStore with parameters
        | UserName |
        | John     |
    Then the last integer result for MySQL UserStore should be 3
...
Scenario: Replace recovery codes with ReplaceCodesAsync without normalized and Guid id
    ...
    And I created users for MySQL UserStore
        | UserName | Email            |
        | John     | john@example.com |
    When I execute "ReplaceCodesAsync" on MySQL UserStore with parameters
        | UserName | RecoveryCodes          |
        | John     | code-1;code-2;code-3  |
    Then the last integer result for MySQL UserStore should be 3
...
Scenario: Redeem recovery code with RedeemCodeAsync without normalized and Guid id
    ...
    And I replaced recovery codes for MySQL UserStore
        | UserName | RecoveryCodes          |
        | John     | code-1;code-2;code-3  |
    When I execute "RedeemCodeAsync" on MySQL UserStore with parameters
        | UserName | Code   |
        | John     | code-1 |
    Then the last boolean result for MySQL UserStore should be "True"
...
Scenario: Get users in role with GetUsersInRoleAsync without normalized and Guid id
    ...
    And I added users to roles for MySQL UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "GetUsersInRoleAsync" on MySQL UserStore with parameters
        | RoleName |
        | Admin    |
    Then the last users result for MySQL UserStore should match
        | UserName | Email            |
...
Scenario: Add user to role with AddToRoleAsync without normalized and Guid id
    ...
    And I created roles for MySQL UserStore
        | Name  |
        | Admin |
    When I execute "AddToRoleAsync" on MySQL UserStore with parameters
        | UserName | RoleName |
        | John     | Admin    |
...
Scenario: Remove user from role with RemoveFromRoleAsync without normalized and Guid id
    ...
    And I added users to roles for MySQL UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "RemoveFromRoleAsync" on MySQL UserStore with parameters
        | UserName | RoleName |
        | John     | Admin    |
    Then the last strings result for MySQL UserStore should match
        | Name |
...
Scenario: Get roles for user with GetRolesAsync without normalized and Guid id
    ...
    And I added users to roles for MySQL UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "GetRolesAsync" on MySQL UserStore with parameters
        | UserName |
        | John     |
    Then the last strings result for MySQL UserStore should match
        | Name  |
...
Scenario: Check role membership with IsInRoleAsync without normalized and Guid id
    ...
    And I added users to roles for MySQL UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "IsInRoleAsync" on MySQL UserStore with parameters
        | UserName | RoleName |
        | John     | Admin    |
    Then the last boolean result for MySQL UserStore should be "True"
...
Scenario: Get role claims for user with GetRoleClaimsAsync without normalized and Guid id
    ...
    And I created roles for MySQL UserStore
        | Name  |
        | Admin |
    And I added role claims for MySQL UserStore
        | RoleName | ClaimType  | ClaimValue   |
        | Admin    | permission | manage-users |
    And I added users to roles for MySQL UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "GetRoleClaimsAsync" on MySQL UserStore with parameters
        | UserName |
        | John     |
    Then the last claims result for MySQL UserStore should match
        | ClaimType  | ClaimValue   |
...
Scenario: Get user and role claims with GetUserAndRoleClaimsAsync without normalized and Guid id
    ...
    And I added users to roles for MySQL UserStore
        | UserName | RoleName |
        | John     | Admin    |
    When I execute "GetUserAndRoleClaimsAsync" on MySQL UserStore with parameters
        | UserName |
        | John     |
    Then the last claims result for MySQL UserStore should match
        | ClaimType  | ClaimValue   |
